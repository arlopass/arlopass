...
/**
 * Page Object for the Arlopass E2E test harness (React + Mantine).
 *
 * Selectors target Mantine component roles, labels, and text content
 * rendered by `apps/e2e-harness/src/App.tsx`.
 */
export class HarnessPage {
    ...
    constructor(page: Page) {
        ...
        // Connection controls
        this.transportProfileSelect = page.getByRole("textbox", { name: "Transport profile" });
        ...
        this.providerSelect = page.getByRole("textbox", { name: "Provider" });
        this.modelSelect = page.getByRole("textbox", { name: "Model" });
    ...
    async waitForFeedback(title: string): Promise<void> {
        await this.page.locator('[class*="Alert-title"]').getByText(title).waitFor({ state: "visible", timeout: 30_000 });
    }
}
