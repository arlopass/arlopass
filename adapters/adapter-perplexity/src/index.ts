...
export type PerplexityConnectionMethodId =
  (typeof PERPLEXITY_CONNECTION_METHOD_IDS)[keyof typeof PERPLEXITY_CONNECTION_METHOD_IDS];

export const PERPLEXITY_CONNECTION_METHODS: readonly ConnectionMethodDescriptor[] = [
  {
...
export const PERPLEXITY_MANIFEST: AdapterManifest = {
  ...
  connectionMethods: PERPLEXITY_CONNECTION_METHODS,
...
function resolveMethodId(methodId: unknown): PerplexityConnectionMethodId | undefined {
  const normalized = normalizeNonEmptyString(methodId);
  if (normalized === PERPLEXITY_CONNECTION_METHOD_IDS.API_KEY) return normalized;
  return undefined;
}
...
function resolveEndpointProfile(
  ...
  const baseUrl = normalizeBaseUrl(
    normalizeNonEmptyString(input["baseUrl"]) ?? PERPLEXITY_DEFAULT_BASE_URL,
  );
...
function resolveCredentialDigest(input: Readonly<Record<string, unknown>>): string {
  ...
  return canonicalizeJsonValue({
    method: PERPLEXITY_CONNECTION_METHOD_IDS.API_KEY,
    apiKeyLength: apiKey.length,
  });
}

export class PerplexityAdapter implements CloudAdapterContractV2 {
  readonly manifest: AdapterManifest = PERPLEXITY_MANIFEST;
  readonly requiredEndpointProfileFields = ["baseUrl"] as const;

  readonly #sessions = new Map<string, PerplexitySession>();
  readonly #credentialRefs = new Map<string, StoredCredentialRef>();
  #sessionCounter = 0;
  #credentialCounter = 0;

  describeCapabilities(): readonly ProtocolCapability[] {
    return PERPLEXITY_MANIFEST.capabilities;
  }

  async listModels(): Promise<readonly string[]> {
    return PERPLEXITY_MODEL_IDS;
  }

  listConnectionMethods(): readonly ConnectionMethodDescriptor[] {
    return PERPLEXITY_CONNECTION_METHODS;
  }
  ...
  async discoverModels(ctx: CloudConnectionContext): Promise<readonly ModelDescriptor[]> {
    ...
    const baseUrl =
      normalizeNonEmptyString(ctx.endpointProfile["baseUrl"]) ?? PERPLEXITY_DEFAULT_BASE_URL;
    return PERPLEXITY_MODEL_DESCRIPTORS.map((descriptor) =>
      Object.freeze({
        ...descriptor,
        baseUrl,
      }),
  ...
  async discoverCapabilities(ctx: CloudConnectionContext): Promise<CapabilityDescriptor> {
    ...
    return {
      capabilities: PERPLEXITY_MANIFEST.capabilities,
      providerId: ctx.providerId,
      methodId: ctx.methodId,
    };
  }
