...
import { Box, Collapse, Divider, Group, ScrollArea, Stack, Text, UnstyledButton } from "@mantine/core";
...
function aggregateModels(providers: WalletProvider[]): ModelEntry[] {
  const modelMap = new Map<string, { name: string; providerKeys: Set<string>; providerCount: number }>();
  for (const provider of providers) {
    ...
    for (const model of provider.models) {
      ...
      if (existing != null) {
        ...
        modelMap.set(model.id, { name: model.name, providerKeys: new Set([pk]), providerCount: 1 });
      }
    }
  }
...
export function ModelsTabContent({ providers }: ModelsTabContentProps) {
  ...
  for (const s of usageSummaries) {
    for (const p of s.byProvider) {
      modelUsageMap[p.modelId] = (modelUsageMap[p.modelId] ?? 0) + p.inputTokens + p.outputTokens;
    }
  }
      ...
      <ScrollArea style={{ flex: 1, minHeight: 0 }} type="scroll" offsetScrollbars scrollbarSize={6}>
        {models.length === 0 ? (
          ...
          <Stack gap={tokens.spacing.sectionGap}>
            {models.map((model) => (
              <ModelCard key={model.id} model={model} tokenUsage={modelUsageMap[model.id] ?? 0} />
            ))}
          </Stack>
        )}
      </ScrollArea>
...
function ModelCard({ model, tokenUsage }: { model: ModelEntry; tokenUsage: number }) {
          ...
          <Stack gap={0} style={{ overflow: "hidden", minWidth: 0 }}>
            ...
            <Group gap={tokens.spacing.metadataGap} wrap="nowrap" style={{ overflow: "hidden" }}>
              <Text fw={500} fz="xs" c={tokens.color.textSecondary} lh="normal" style={{ whiteSpace: "nowrap" }}>
                {model.providerCount} {model.providerCount === 1 ? "provider" : "providers"} available
              </Text>
              {tokenUsage > 0 && (
                <>
                  <MetadataDivider />
        ...
        <Box style={{ padding: `0 ${tokens.spacing.cardPadding}px ${tokens.spacing.cardPadding}px` }}>
          ...
          <Stack gap={8}>
            <Group justify="space-between">
              ...
              <Text fz="xs" fw={500} c={tokens.color.textPrimary} truncate maw={180}>{model.id}</Text>
            </Group>
            <Group justify="space-between">
              <Text fz="xs" c={tokens.color.textSecondary}>Providers</Text>
              <Text fz="xs" fw={500} c={tokens.color.textPrimary}>{model.providerCount} available</Text>
            </Group>
            {tokenUsage > 0 && (
              <Group justify="space-between">
