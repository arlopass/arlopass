...
export function validateGeminiConnectorInput(
  config: Readonly<Record<string, string>>,
): ConnectorValidationResult {
  const methodId = normalizeText(config["methodId"], DEFAULT_METHOD_ID);
  if (methodId !== "gemini.api_key" && methodId !== "gemini.oauth_access_token") {
    return { ok: false, message: "Unsupported Gemini methodId." };
  }
  ...
  if (methodId === "gemini.api_key") {
    if (normalizeText(config["apiKey"]).length === 0) {
      return { ok: false, message: "API key is required for gemini.api_key." };
    }
    return { ok: true };
  }

  if (normalizeText(config["accessToken"]).length === 0) {
    return { ok: false, message: "Access token is required for gemini.oauth_access_token." };
  }
  return { ok: true };
}

export function sanitizeGeminiConnectorMetadata(
  config: Readonly<Record<string, string>>,
): Readonly<Record<string, string>> {
...
export function createCloudGeminiConnector(
  deps: CloudConnectorDependencies,
): ConnectorDefinition {
  return {
    id: CLOUD_GEMINI_CONNECTOR_ID,
    ...
    fields: [
      ...
      },
      {
        ...
        options: [
          { value: "gemini.api_key", label: "API Key" },
          { value: "gemini.oauth_access_token", label: "OAuth Access Token" },
        ],
      },
      {
        key: "baseUrl",
      ...
      {
        key: "apiKey",
        label: "API Key (for gemini.api_key)",
        type: "password",
        required: false,
        maxLength: 500,
      },
      {
        key: "accessToken",
        label: "Access Token (for gemini.oauth_access_token)",
        type: "password",
        required: false,
        maxLength: 500,
      },
    ...
    async testConnection(config): Promise<ConnectionTestResult> {
      const validation = validateGeminiConnectorInput(config);
      if (!validation.ok) {
        return {
          ok: false,
          status: "attention",
      ...
      const metadata = sanitizeGeminiConnectorMetadata({
        ...config,
        providerId: DEFAULT_PROVIDER_ID,
      ...
      return {
        ok: true,
        status: "connected",
        message: "Gemini connection validated through native bridge.",
    ...
    sanitizeMetadata(config): Readonly<Record<string, string>> {
      return sanitizeGeminiConnectorMetadata({
        ...config,
        providerId: DEFAULT_PROVIDER_ID,
      });
    },
