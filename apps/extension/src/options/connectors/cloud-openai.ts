...
export const CLOUD_OPENAI_CONNECTOR_ID = "cloud-openai";

const DEFAULT_NATIVE_HOST_NAME = "com.byom.bridge";
const DEFAULT_PROVIDER_ID = "openai";
const DEFAULT_METHOD_ID = "openai.api_key";
...
export function sanitizeOpenAiConnectorMetadata(
  config: Readonly<Record<string, string>>,
): Readonly<Record<string, string>> {
  ...
  const organization = normalizeText(config["organization"]);
  const project = normalizeText(config["project"]);
...
> {
  ...
  const completeResponse = await deps.sendNativeMessage(nativeHostName, {
    ...
    input: {
      ...
      baseUrl: normalizeUrl(config["baseUrl"] ?? DEFAULT_BASE_URL),
      organization: normalizeText(config["organization"]),
      project: normalizeText(config["project"]),
...
export function createCloudOpenAiConnector(
  deps: CloudConnectorDependencies,
): ConnectorDefinition {
  return {
    id: CLOUD_OPENAI_CONNECTOR_ID,
    label: "OpenAI (Cloud)",
    type: "cloud",
    defaultName: "OpenAI",
    fields: [
    ...
    async testConnection(config): Promise<ConnectionTestResult> {
      const validation = validateOpenAiConnectorInput(config);
      if (!validation.ok) {
        return {
          ok: false,
          status: "attention",
      ...
      const metadata = sanitizeOpenAiConnectorMetadata({
        ...config,
        providerId: DEFAULT_PROVIDER_ID,
      ...
      return {
        ok: true,
        status: "connected",
        message: "OpenAI connection validated through native bridge.",
    ...
    sanitizeMetadata(config): Readonly<Record<string, string>> {
      return sanitizeOpenAiConnectorMetadata({
        ...config,
        providerId: DEFAULT_PROVIDER_ID,
      });
    },
