...
export function InteractiveProvider({ children }: { children: ReactNode }) {
  ...
  const [injAvail, setInjAvail] = useState(
  ...
  const sp = useMemo(
    () => provs.find((p) => p.providerId === selProv) ?? null,
    [provs, selProv],
  );
  ...
  const syncState = useCallback(
    () => setState(clientRef.current?.state ?? "disconnected"),
    [],
  );
  ...
  const run = useCallback(
    async (name: string, op: () => Promise<void>) => {
      ...
      try {
        ...
        setFb(errFb(name, e));
        log("error", `${name} failed`, errStr(e));
      } finally {
        setBusy(null);
        syncState();
        setInjAvail(getInjectedTransport() !== null);
      }
    },
    [busy, log, syncState],
  );
  ...
  const doConnect = useCallback(async () => {
    ...
    setFb({
      kind: "success",
      title: "Connected",
      message: `Session ${res.sessionId}`,
    });
    log("success", `Connected via ${r.source}`);
    if (r.warning) {
      setFb({ kind: "info", title: "Fallback", message: r.warning });
      log("info", "Fallback", r.warning);
    }
  ...
  const doList = useCallback(async () => {
    ...
    setFb({
      ...
      message: `${r.providers.length} available`,
    });
    return f && f.models[0]
      ? { providerId: f.providerId, modelId: f.models[0] }
      : null;
  }, [log]);
  ...
  const doSelect = useCallback(
    async (pId?: string, mId?: string) => {
      ...
      setFb({
        kind: "success",
        title: "Selected",
        message: `${r.providerId}/${r.modelId}`,
      });
