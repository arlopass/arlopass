...
function toChunks(value: string): readonly string[] {
  ...
  const chunks: string[] = [];
  for (let cursor = 0; cursor < normalized.length; cursor += STREAM_CHUNK_SIZE) {
...
export function createDemoTransport(mode: DemoTransportMode): ArlopassTransport {
  return {
    async request<TRequestPayload, TResponsePayload>(
      ...
      if (capability === "session.create") {
        if (hasOwnKey(request.envelope.payload, "appId")) {
          ...
          return {
            envelope: createResponseEnvelope<ConnectPayload, ConnectResponsePayload>(
        ...
        return {
          envelope: createResponseEnvelope<SelectProviderInput, SelectProviderResponsePayload>(
      ...
      if (capability === "provider.list") {
        ...
        return {
          envelope: createResponseEnvelope<ProviderListPayload, ProviderListResponsePayload>(
      ...
      if (capability === "chat.completions") {
        ...
        return {
          envelope: createResponseEnvelope<ChatSendPayload, ChatSendResponsePayload>(
    ...
    async stream<TRequestPayload, TResponsePayload>(
      ...
      async function* streamGenerator(): AsyncIterable<TransportResponse<TResponsePayload>> {
        for (let index = 0; index < chunks.length; index += 1) {
          ...
          yield {
            envelope: createResponseEnvelope<ChatStreamPayload, ChatStreamResponsePayload>(
