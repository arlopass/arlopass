...
const reExportImport = `// Re-exported from @byom-ai/web-sdk — no need to install web-sdk separately
import type {
  ChatMessage,
  ChatRole,
  ClientState,
...
const providerPropsDef = `type BYOMProviderProps = Readonly<{
  appId?: string;
  appSuffix?: string;
  appName?: string;
  ...
  defaultModel?: string;
  autoConnect?: boolean;
  onError?: (error: BYOMSDKError) => void;
...
export default function ReactTypes() {
  return (
    <Stack gap="lg">
      ...
      <Text>
        All types are importable from <InlineCode>@byom-ai/react</InlineCode>.
        This includes React-specific types and re-exports from{" "}
        <InlineCode>@byom-ai/web-sdk</InlineCode>.
      </Text>
      ...
      <Text fz="sm">Opaque string identifier for tracked messages (UUID v4).</Text>

      <Title order={4}>TrackedChatMessage</Title>
      <CodeBlock code={trackedChatMessageDef} language="tsx" />
      <Text fz="sm">
        Immutable message object returned by <InlineCode>useChat</InlineCode> and{" "}
        <InlineCode>useConversation</InlineCode>. Extends the base{" "}
      ...
      <Text fz="sm">
        Attached to a <InlineCode>TrackedChatMessage</InlineCode> when the assistant
        invokes tools.
      </Text>
      ...
      <Text fz="sm">
        Overloaded subscribe function returned by <InlineCode>useConversation</InlineCode>.
        Supports targeted subscriptions by message ID.
      </Text>

      <Title order={4}>ChatSubscribeNoTools</Title>
      <CodeBlock code={chatSubscribeNoToolsDef} language="tsx" />
      <Text fz="sm">
        Restricted subscribe type returned by <InlineCode>useChat</InlineCode> (no tool events).
      </Text>

      <Title order={4}>BYOMProviderProps</Title>
      ...
      <Text fz="sm">Internal store snapshot exposed by the provider context.</Text>

      {/* Re-exported types */}
      <Divider />
      <Title order={3}>Re-exported from @byom-ai/web-sdk</Title>
      <Text>
        These types are re-exported so you only need the{" "}
        <InlineCode>@byom-ai/react</InlineCode> package.
      </Text>
