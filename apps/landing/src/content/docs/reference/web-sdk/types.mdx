...
```tsx
type TransportStream<TPayload = unknown> = AsyncIterable<TransportResponse<TPayload>>;
```
...
```tsx
type ChatStreamChunkPayload = Readonly<{ type: "chunk"; delta: string; index: number }>;
```
...
```tsx
type ToolPrimingStartEvent = Readonly<{ type: "tool_priming_start"; message: string }>;
```

#### ToolPrimingMatchEvent

```tsx
type ToolPrimingMatchEvent = Readonly<{ type: "tool_priming_match"; tools: readonly string[] }>;
```
